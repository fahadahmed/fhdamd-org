...
<MermaidDiagramBlock label="Astro output modes · Mermaid" caption="One framework, two output modes — the choice is per-project, not a ceiling on the framework" source={`flowchart TD
    ...
    accDescr: This site renders with output static — HTML is pre-rendered at build time, so there is no server running at request time. Riqa renders with output server as a fully authenticated SaaS product with a dashboard, billing, and file processing.
    A[Astro] -->|output: static| B["fhdamd.dev<br/>pre-rendered at build time<br/>no server at request time"]
