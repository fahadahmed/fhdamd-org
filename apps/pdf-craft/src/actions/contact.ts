...
export const contact = {
  sendMessage: defineAction({
    ...
    handler: async (input) => {
      ...
      if (!isHuman) {
        log.warn('📬 contact-form: captcha failed', { feature: 'contact', status: 'fail' });
        return { success: false, error: 'Captcha verification failed. Please try again.' };
      }

      try {
        await sendViaResend({ fromName: name, fromEmail: email, subject: subjectLabel, message });
        log.business('📬 contact-form-sent', { feature: 'contact', status: 'success' });
        return { success: true };
      } catch (error) {
